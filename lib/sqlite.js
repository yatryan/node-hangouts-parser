    ...
    for (var i = 0; i < conversations.length; i++) {
      ...
      for (var j = 0; j < messages.length; j++) {
        ...
        if (message.personID === UNKNOWN && unknownArray.indexOf(message.googleID) === -1) {
          ...
          if (message.googleID === '113883621335386500770') {

            Hangouts.getPersonForMessage(message).then(function(res) {
            //   var newPerson = new Person(res.id, res.name);
            //   newPerson._google.push(res.email);
            //
            //   // if (res.name === 'Melissa Rose') {
  ...
  var p = new Promise(function (resolve, reject) {
    ...
    for (var i = 0; i < threads.length; i++) {
      ...
      for (var j = 0; j < messages.length; j++) {
        ...
        if (message.personID === UNKNOWN && unknownArray.indexOf(message.facebookID) === -1) {
          unknownArray.push(message.facebookID);
          console.log('Could not find person for ',message.sender, '(',message.facebookID,')');

          Facebook.getPersonForMessage(message).then(function(res) {
            var newPerson = new Person(res.id, res.name);
