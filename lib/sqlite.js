...
var loadPeople = function() {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    results.push(db.allAsync('SELECT * FROM  `sg_google` WHERE  `personID` = ?;').all(tempPerson.id));
    results.push(db.allAsync('SELECT * FROM  `sg_facebook` WHERE  `personID` = ?;').all(tempPerson.id));  
  }
